...
RUN apt update && apt -yq upgrade \
    && apt -yq install zip unzip bash-completion build-essential ruby ruby-dev python3-pip \
    ninja-build htop iputils-ping jq less locales man-db nano vim ripgrep software-properties-common \
    clojure golang cmake apache2 nginx nginx-extras nginx-doc php php-all-dev php-bcmath php-common \
...
RUN mkdir -p /var/run/nginx \
    ...
    && chown -R gitpod:gitpod /etc/apache2 /var/log/apache2 \
